...
@section('content')
    <h1 class="text-xl font-semibold mb-4">Справочники — группы тарифов</h1>
    ...
    <x-ui.card>
        <x-slot:actions>
            <div class="flex items-center gap-3">
                ...
                @if ($items->isNotEmpty())
                    @can('create', \App\Models\TariffGroup::class)
                        <x-ui.button :href="route('admin.tariff_groups.create')" variant="primary" size="sm">
                            Добавить
                        </x-ui.button>
                    @endcan
                @endif
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <div class="flex justify-end gap-2">
                                @can('update', $g)
                                    <x-ui.button :href="route('admin.tariff_groups.edit', $g)" variant="primary" size="sm">
                                        Править
                                    </x-ui.button>
                                @endcan

                                @can('toggle', $g)
                                    <form action="{{ route('admin.tariff_groups.toggle', $g) }}" method="post">
                                        @csrf
                                        @method('PATCH')
                                ...
                                @can('delete', $g)
                                    <form action="{{ route('admin.tariff_groups.destroy', $g) }}" method="post"
